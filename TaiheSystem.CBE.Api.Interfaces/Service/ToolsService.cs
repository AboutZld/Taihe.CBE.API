    ...
    public class ToolsService : DbContext, IToolsService
    {
        ...
        public bool CreateModels(string strPath, string strNameSpace, string tableName, string strInterface, bool blnSerializable = false)
        {
            ...
            try
            {
                ...
                IDbFirst.IsCreateDefaultValue().IsCreateAttribute().SettingClassDescriptionTemplate(p=>p = classdescriptionTemplate)
                    //.SettingConstructorTemplate(p => p = "              this.{PropertyName} =  {DefaultValue};")
                    //.SettingClassTemplate(p => p = classTemplate)
                    .SettingNamespaceTemplate(old =>
                    {
                        return old + "using System.ComponentModel.DataAnnotations;\n";//修改using命名空间
                    })
        ...
        /// </summary>
        /// <returns></returns>
        public List<string> GetAllViews(string name = "")
        {
            if(name != "")
            {
                return Db.DbMaintenance.GetViewInfoList().Where(i =>i.Name == name).Select(it => it.Name).ToList();
            }
            return Db.DbMaintenance.GetTableInfoList().Select(it => it.Name).ToList();
        }
