...
namespace TaiheSystem.CBE.Api.Hostd.Controllers.Evaluation
{
    ...
    [ApiController]
    public class DataRecoveryController : BaseController
    {
        /// <summary>
        /// 日志管理接口
        /// </summary>
...
").ToPage<view_dataRecovery>(predicate.ToExpression(), parm);
        ...
        /// <summary>
        /// 获取资料回收详情
        /// Power = PRIV_RECOVERY_DETAIL
        /// </summary>
        /// <param name="parm">ids</param>
        /// <returns></returns>
        [HttpGet]
        [Authorization(Power = "PRIV_RECOVERY_DETAIL")]
...
inner join Abi_SysStandard s on s.ID = ci.ItemStandardID where cis.ContractPlanID = '{plandata.ID}' ").ToList();
        ...
        /// <summary>
        /// 资料回收完成
        /// Power = PRIV_RECOVERY_FINISH
        /// </summary>
        /// <param name="id">编码</param>
        /// <returns></returns>
        [HttpPost]
        [Authorization(Power = "PRIV_RECOVERY_FINISH")]
        public IActionResult SubmitConfirm(DataRecoverySubmitDto parm)
        {
            ...
            using (SqlSugarClient db = Core.DbContext.CurrentDB)
            {
                ...
                try
                {
                    ...
                    parameters.Add(new SugarParameter("ReviewerID", parm.ReviewerID)); //阅卷老师ID
                    parameters.Add(new SugarParameter("ReviewerName", parm.ReviewerName)); //阅卷老师
                    parameters.Add(new SugarParameter("DataRecoveryDate", parm.DataRecoveryDate)); //资料回收日期
                    parameters.Add(new SugarParameter("DataRecoveryRemark", parm.DataRecoveryRemark)); //资料回收备注
                    parameters.Add(new SugarParameter("ReplenishRemark", parm.ReplenishRemark)); //待补充备注

                    Step.Submit(db, maincontract, "Biz_Contract_Plan", "ID", "status", "304", parameters, UpdatePlanAfterSubmitted, "资料回收完成");
        ...
        {
            if (db.Ado.ExecuteCommand(@"UPDATE Biz_Contract_Plan SET Status = @Node_To,ReviewerID=@ReviewerID,ReviewerName=@ReviewerName,DataRecoveryDate=@DataRecoveryDate,DataRecoveryRemark=@DataRecoveryRemark,ReplenishRemark=@ReplenishRemark
WHERE ID = @Biz_Contract_Plan_ID AND Status = @Node_From", paramters) == 0)
        ...
        public IActionResult SubmitReturn(DataRecoverySubmitDto parm)
        {
            var userinfo = _tokenManager.GetSessionInfo();
            var maincontract = _contractitemplanService.GetId(parm.id);
            ...
            using (SqlSugarClient db = Core.DbContext.CurrentDB)
            {
                ...
                try
                {
                    ...
                    Step.Submit(db, maincontract, "Biz_Contract_Plan", "ID", "status", "305", parameters, UpdatePlanAfterSubmitted, "资料待补充");
                    Core.DbContext.CommitTran();
                    return toResponse("提交成功");
                }
                catch (Exception ex)
