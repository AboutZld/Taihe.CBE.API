...
namespace TaiheSystem.CBE.Api.Hostd.Controllers.Evaluation
{
    ...
    public class EvaluationController : BaseController
    {
        ...
        public IActionResult Query([FromBody] EvaluationDto parm)
        {
            ...
            //状态 0-未评定 1- 待整改 2-已整改 3-评定未通过 4-评定通过
            switch (parm.status)
            {
                case 0:
                    ...
                    break;
                case 1:
                    predicate = predicate.And(m => m.status == 30033 && m.ItemStatus == 20015);
                    break;
                case 2:
                    predicate = predicate.And(m => m.status == 30035 && m.ItemStatus == 20018);
                    break;
                case 3:
                    predicate = predicate.And(m => m.status == 30040 && m.ItemStatus == 20023);
                    break;
                case 4:
                    predicate = predicate.And(m => m.status == 30044 && m.ItemStatus == 20020);
                    break;
                default:
